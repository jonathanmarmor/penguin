...
from build_chord_options import chordtypes_on_roots, containing, chords_in_scale
...
##                                'Katie', 'Beth', 'Jason']
    ...
    movement.harmonies = []
    movement.notes = {}
    movement.tempo_bpm = random.choice(range(54,91,4))
    ...
    movement.duration_of_segments = 32
...
def choose_sections_order(movement):
    ...
    if movement.tempo_bpm == 54: num_segs = random.choice([6, 7, 8, 9])
    if movement.tempo_bpm == 58: num_segs = random.choice([7, 8, 9, 10])
    if movement.tempo_bpm == 62: num_segs = random.choice([7, 8, 9, 10])
    if movement.tempo_bpm == 66: num_segs = random.choice([7, 8, 9, 10, 11])
    if movement.tempo_bpm == 70: num_segs = random.choice([8, 9, 10, 11])
    if movement.tempo_bpm == 74: num_segs = random.choice([8, 9, 10, 11])
    if movement.tempo_bpm == 78: num_segs = random.choice([9, 10, 11])
    if movement.tempo_bpm == 82: num_segs = random.choice([9, 10, 11])
    if movement.tempo_bpm == 86: num_segs = random.choice([9, 10, 11])
    if movement.tempo_bpm == 90: num_segs = random.choice([10, 11])
...
def make_section_type(name, movement):
    ...
    scale = random.choice([(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),
                           (0,2,4,7,9),(0,2,4,7,9),
                           #(0,1,4,6,8,11),
                           #(0,2,5,6,7,9),(0,2,5,6,7,9),
                           ])
...
def make_harmony_options(segment):
    ...
    segment.harmony_options = chords_in_scale(opts, segment.scale)
...
def make_harmonies(segment):
    ...
    for d in segment.harmonic_rhythm:
        ...
        s += d
        harmony.pitches = random.choice(segment.harmony_options) #get_chord(segment, previous_chord)
        ...
        harmony.scale = segment.scale #random.choice(containing(segment.section_type.scale_options, harmony.pitches))
...
def get_next_melody_pitch(prevs, harmony, scale, register):
    ...
    if prevs[-1] + 1 in chord_pitches: preferred.append(prevs[-1] + 1)
    if prevs[-1] + 2 in chord_pitches: preferred.append(prevs[-1] + 2)
    if prevs[-1] + 3 in chord_pitches: tertiary.append(prevs[-1] + 3)
    if prevs[-1] + 4 in chord_pitches: tertiary.append(prevs[-1] + 4)
    if prevs[-1] + 5 in chord_pitches: tertiary.append(prevs[-1] + 5)
    if prevs[-1] - 1 in chord_pitches: preferred.append(prevs[-1] - 1)
    if prevs[-1] - 2 in chord_pitches: preferred.append(prevs[-1] - 2)
    if prevs[-1] - 3 in chord_pitches: tertiary.append(prevs[-1] - 3)
    if prevs[-1] - 4 in chord_pitches: tertiary.append(prevs[-1] - 4)
...
def get_melody_rhythm_durs(duration):
    opts = {}
    opts[4] = [ [(False,4)], [(True,4)], [(True,4)], [(True,2),(True,2)] ]
    opts[8] = [ [(True,8)], [(True,8)], [(False,8)], [(True,4),(True,4)], [(True,4),(True,4)], [(True,4),(True,2),(True,2)], [(True,2),(True,2),(True,2),(True,2)], [(True,4),(False,4)] ]
    opts[12] = [ 
        [(True,12)], [(False,12)], 
