...
from Movements.Mvmt7_make_ensembles import make_ensembles
...
##                                'Katie', 'Beth', 'Jason']
    ...
    movement.harmonies = []
    movement.notes = {}
    movement.tempo_bpm = random.choice(range(66,87,4))
    ...
    movement.duration_of_segments = 512 #
...
def choose_sections_order(movement):
    ...
    if movement.tempo_bpm == 66: num_segs = 8
    if movement.tempo_bpm == 70: num_segs = random.choice([8, 9])
    if movement.tempo_bpm == 74: num_segs = 9
    if movement.tempo_bpm == 78: num_segs = random.choice([9, 10])
    if movement.tempo_bpm == 82: num_segs = 10
    if movement.tempo_bpm == 86: num_segs = random.choice([10, 11])
...
def make_section_type(name, movement):
    ...
    section_type.num_of_segments = random.choice([1,1,1,2,2,4,4])
...
##    segment.scale_options = containing(all_scales, [segment.movement.drone_pc])
    segment.scale_options = chordtypes_on_roots([(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),(0,2,4,5,7,9,11),(0,2,4,7,9),(0,2,4,7,9),(0,1,4,6,8,11)])
